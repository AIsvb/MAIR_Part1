...
def get_train_val_data(filename, remove_duplicates = False):
    ...
    sentences_train, sentences_val, labels_train, labels_val = train_test_split(sentences, labels, test_size=0.15,
                                                                                random_state=42)
...
if __name__ == "__main__":
    ...
    # Initiating and training the classifier based on the chosen ML algorithm
    if(mlAlgorithm == "svm"):
        classifier = svm.SVC()
    else:
        classifier = RandomForestClassifier(n_estimators=100, random_state=42)
    
    classifier.fit(X_train[:-1], labels_train)
    dump(classifier, CLASSIFIER_DIR) # Save the classifier

    # Validation stage
    val_predictions = classifier.predict(X_val)

    accuracy = accuracy_score(labels_val, val_predictions)
    report = classification_report(labels_val, val_predictions, zero_division=0)
